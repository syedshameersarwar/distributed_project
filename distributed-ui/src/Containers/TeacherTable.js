...
class TeacherTable extends React.Component {
  ...
  componentDidMount = () => {
    fetch(
      `${apiEndpoint}/sync/teacher/${this.state.page}/${this.state.unit}?host=${this.props.host}`
    )
      .then(res => res.json())
      .then(payload => {
  ...
  delete = record => {
    ...
    fetch(`${apiEndpoint}/sync/teacher`, {
      method: "DELETE",
      headers: {
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      if (index > -1) {
        ...
        const { id, name, contact, email, city, district, desc } = {
          ...item,
          ...row
        };
  ...
  paginate = (page, unit) => {
    ...
    this.cancel();
    fetch(
      `${apiEndpoint}/sync/teacher/${this.state.page}/5?host=${this.props.host}`
    )
      .then(res => res.json())
      .then(payload => {
        const { length, data } = payload;
...
export default Form.create()(TeacherTable);
